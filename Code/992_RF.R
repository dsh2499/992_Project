...
predictions_rft <- predict(rf_model, type="prob")
my_df <- data.frame(Column1 = numeric(), Column2 = numeric(),Column3=numeric())
for (val in t_vals)
{
  predicted_classes <- ifelse(predictions_rft[,2] > val, 'PE', 'No.PE')
...
#Threshold of 0.092 chosen

# Make predictions on the test data
predictions_rf <- predict(rf_model, newdata = test_data, type="prob")

# Convert probabilities to class labels
predicted_classes <- ifelse(predictions_rf[,2] >.092, 'PE', 'No.PE')
...
predictions_t <- predict(rf_model2, type="prob")
my_df <- data.frame(Column1 = numeric(), Column2 = numeric(),Column3=numeric())
for (val in t_vals)
{
...
# Make predictions on the test data
predictions <- predict(rf_model2, newdata = test_data, type="prob")

# Convert probabilities to class labels
predicted_classes <- ifelse(predictions[,2] > .5, 'PE', 'No.PE')
